...
std::string DownloadManager::defaultDownloadDir() {
    ...
    const auto fallback = joinPath(joinPath(conf.getHomePath(), "Downloads"), "wiliwili");
...
void DownloadManager::addTask(DownloadTask task) {
    ...
    if (task.dir.empty()) {
        auto base = defaultDownloadDir();
        cpr::fs::create_directories(base);
        auto stem = (task.bvid.empty() ? std::string{"pgc"} : task.bvid) + "_" + std::to_string(task.cid);
        auto candidate = joinPath(base, stem);
        if (cpr::fs::exists(candidate)) candidate += "_" + std::to_string(task.created_at);
        task.dir = candidate;
    }
    if (task.video_file.empty()) task.video_file = task.is_dash ? "video.m4s" : "video.flv";
    if (task.is_dash && !task.audio_urls.empty() && task.audio_file.empty()) task.audio_file = "audio.m4s";
...
void DownloadManager::startNextTask() {
    ...
    for (const auto& id : starts) cpr::async([this, id]() { runTask(id); });
}
...
bool DownloadManager::downloadFile(const std::string& url, const std::string& filepath,
    ...
    int64_t limit = 0;
    try {
        auto s = ProgramConfig::instance().getSettingItem(SettingItem::DOWNLOAD_SPEED_LIMIT, std::string{"0"});
        double mb = std::stod(s); if (mb > 0) limit = static_cast<int64_t>(mb * 1024.0 * 1024.0);
    } catch (...) {}
    ...
    session->SetWriteCallback(cpr::WriteCallback([&](std::string data, intptr_t) -> bool {
        ...
        if (limit > 0) {
            ...
            } else if (elapsed >= 1.0) { windowStart = now; windowBytes = 0; }
        }
        auto now = std::chrono::steady_clock::now();
...
void DownloadManager::writeReadme(const DownloadTask& task) {
    try {
        ...
        if (task.muxed && !task.output_file.empty()) {
            ...
            f << "ffmpeg was not available or muxing failed. You can merge without re-encoding:\n";
            f << "ffmpeg -i " << task.video_file << " -i " << task.audio_file << " -map 0:v:0 -map 1:a:0 -c copy video.mp4\n";
        } else {
            f << "Playable/source file: " << task.video_file << "\n";
        }
        f << "\nRaw source URLs and stream IDs are preserved in source.json.\n";
        f << "Metadata is preserved in info.json; danmaku is saved as danmaku.xml when available.\n";
    } catch (...) {}
}
...
void DownloadManager::saveMetadata(const DownloadTask& task, bool completed) {
    try {
        ...
        nlohmann::json source = {
            {"format", task.is_dash ? "dash" : "flv"}, {"source_page_url", task.source_page_url},
            {"title", task.title}, {"owner_name", task.owner_name}, {"cover_url", task.cover_url},
            {"bvid", task.bvid}, {"aid", task.aid}, {"cid", task.cid}, {"is_pgc", task.is_pgc}, {"created_at", task.created_at},
            {"video", {{"quality_id", task.quality}, {"quality_desc", task.quality_desc}, {"codec_id", task.video_codec_id},
                       ...
                       {"file", task.video_file}, {"urls", task.video_urls}}},
            {"audio", {{"id", task.audio_id}, {"desc", task.audio_desc}, {"codec_id", task.audio_codec_id},
                       {"bandwidth", task.audio_bandwidth}, {"file", task.audio_file}, {"urls", task.audio_urls}}},
            {"flv_segments", task.flv_segments}
        };
        std::ofstream sf(joinPath(task.dir, "source.json"), std::ios::trunc); if (sf.is_open()) sf << source.dump(2);
        nlohmann::json info = {
            {"id", task.id}, {"title", task.title}, {"owner_name", task.owner_name}, {"cover_url", task.cover_url},
            {"bvid", task.bvid}, {"aid", task.aid}, {"cid", task.cid}, {"quality", task.quality}, {"quality_desc", task.quality_desc},
            {"audio_id", task.audio_id}, {"audio_desc", task.audio_desc}, {"is_dash", task.is_dash}, {"is_pgc", task.is_pgc},
            ...
            {"download_dir", task.dir}, {"muxed", task.muxed}, {"created_at", task.created_at}, {"completed", completed},
            {"source_file", "source.json"}, {"cover_file", "cover.jpg"}, {"danmaku_file", "danmaku.xml"}
        };
        std::ofstream f(joinPath(task.dir, "info.json"), std::ios::trunc); if (f.is_open()) f << info.dump(2);
...
void DownloadManager::saveDanmaku(const DownloadTask& task) {
    ...
    BILI::get_danmaku(task.cid, [dir](const std::string& xml) {
        std::ofstream f(joinPath(dir, "danmaku.xml"), std::ios::trunc); if (f.is_open()) f << xml;
    }, [](BILI_ERR) { brls::Logger::error("DownloadManager: danmaku failed: {}", error); });
}
...
void DownloadManager::runTask(const std::string& id) {
    ...
    // CDN URLs expire. Refresh them when a queued/resumed/retried task actually starts.
    // If the refresh endpoint is temporarily unavailable, the stored source URLs are still attempted.
    refreshSource(work);
    saveMetadata(work, false);
    if (!work.cover_url.empty()) {
        auto coverPath = joinPath(work.dir, "cover.jpg");
        if (!cpr::fs::exists(coverPath)) {
            try {
                auto s = bilibili::HTTP::createSession(); s->SetUrl(cpr::Url{work.cover_url}); s->SetTimeout(cpr::Timeout{10000});
                auto r = s->Get(); if (!r.error && r.status_code == 200) { std::ofstream f(coverPath, std::ios::binary); if (f.is_open()) f.write(r.text.data(), r.text.size()); }
            } catch (...) {}
        }
    }
    bool success = work.is_dash ? downloadDash(work) : downloadFlv(work);
    if (success && work.is_dash) tryMuxDash(work);
    ...
    std::string title;
    {
        ...
        runningTasks.erase(id);
        for (auto& t : tasks) if (t.id == id) {
            // A quick pause->resume replaces the flags. Do not let the old worker clobber the new generation.
            ...
            t.output_file = work.output_file; t.muxed = work.muxed;
            ...
            else if (success) { t.status = DownloadTaskStatus::COMPLETED; completed = true; title = t.title; }
            else t.status = DownloadTaskStatus::FAILED;
